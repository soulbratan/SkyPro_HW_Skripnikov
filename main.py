    ...
    if len(result_5) == 0:
        ...
        for i in result_5:
            ...
            if i.get("description").lower() == "открытие вклада":
                ...
                second_line = (
                    f"{widget.mask_account_card(i.get('from', "Новый"))} -> {widget.mask_account_card(i.get('to'))}\n"
                )
            try:
                third_line = (
                    f"Сумма: {i['operationAmount'].get('amount')} {i['operationAmount'].get('currency').get('name')}\n"
                )
            except KeyError:
