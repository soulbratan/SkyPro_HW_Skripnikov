...
def get_mask_card_number(card_number: Union[str, int]) -> str:
    ...
    logger.info(f"Func <{get_mask_card_number.__name__}> started. Input: ****{str(card_number)[-4:]}")
    card_number = str(card_number)
    clean_number = card_number.replace(" ", "")
    if len(clean_number) != 16 or not clean_number.isdigit():
        logger.error(f"Wrong card number length. {len(clean_number)} != 16. Or number not contain only numbers.")
    ...
    logger.info(f"Func <{get_mask_card_number.__name__}> successfully completed.")
...
def get_mask_account(account_number: Union[str, int]) -> str:
    ...
    logger.info(f"Func <{get_mask_account.__name__}> started. Input: ****{str(account_number)[-4:]}")
    account_number = str(account_number)
    if len(account_number) != 20 or not account_number.isdigit():
        logger.error(f"Wrong card number length. {len(account_number)} != 20. Or number not contain only numbers.")
        raise ValueError("Номер карты должен содержать 20 цифр")
    hidden_part: str = "**"
    masked_number: str = f"{hidden_part}{account_number[-4:]}"
    logger.info(f"Func <{get_mask_account.__name__}> successfully completed.")
    return masked_number
